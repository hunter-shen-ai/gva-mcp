    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="函数名称:" prop="funcName">
          <el-input v-model="formData.funcName" :clearable="true"  placeholder="请输入函数名称" />
       </el-form-item>
        <el-form-item label="函数描述:" prop="funcDescription">
          <el-input v-model="formData.funcDescription" :clearable="true"  placeholder="请输入函数描述" />
       </el-form-item>
        <el-form-item label="函数类型:" prop="funcType">
          <el-input v-model="formData.funcType" :clearable="true"  placeholder="请输入函数类型" />
       </el-form-item>
        <el-form-item>
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findMcpServerTool({ ID: route.query.id })
      if (res.code === 0) {
